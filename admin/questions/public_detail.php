...
//           $url = $router->createUrl('Packages/detail', ["id" => $id_pack, "created_by" => $user->getID()]);
            $router->redirect("questions/public_index");
...
<form action="<?= $router->createUrl('questions/public_detail',["id"=>$ques[0]["id"], "pack" => $id_pack]) ?>" method="POST">
    <div>
    ...
    <a href="<?= $router->createUrl('questions/public_index')  ?>">CANCEL</a>
    <?php  } ?>
    <?php  } ?>
    <?php if (!$router->getGET("id")){ ?>
        ...
        <input type="submit" name="submit" value="Post">
        <a href="<?= $router->createUrl('Packages/public_detail')  ?>">CANCEL</a>
        <?php  } ?>
